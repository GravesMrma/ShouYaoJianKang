...
object DateUtils {
     ...
     * @return
     */
    fun getToDay(cl: Calendar): Calendar {
        val day = cl.get(Calendar.DATE)
     ...
     */
    fun getToDay(): String {
        return SimpleDateFormat(FORMAT_SHORT).format(Date(System.currentTimeMillis()))
    }
    ...
    @Throws(ParseException::class)
    private fun longToDate(currentTime: Long, formatType: String): Date {
    ...
    fun stringFormat(strTime: String, formatType: String = FORMAT_SHORT): String {
        var date: Date? = null // String类型转成date类型
        try {
            date = stringToDate(strTime, datePattern)
        } catch (e: ParseException) {
            e.printStackTrace()
        }
    ...
    fun getTimeCompareSize(startTime: String, endTime: String): Int {
        var i = 0
        ...
        try {
            ...
            when {
                date2.time < date1.time -> i = 1
                date2.time == date1.time -> i = 2
                date2.time > date1.time -> i = 3
            }
        } catch (e: ParseException) {
            e.printStackTrace()
        }
