...
class SureOrderActivity : BaseMvpActivity<SureOrderPresenter>(), SureOrderView {
    ...
    override fun onDoneCartResult(result: SureOrderBean) {
        ...
        cartid = ""
        for (product in result.storedata.first().product_list) {
            cartid = cartid + product.cartid.toString() + ","
        }
    }
        ...
        adapter = object : BaseQuickAdapter<Storedata, BaseViewHolder>(R.layout.item_order_sure, list) {
            override fun convert(helper: BaseViewHolder?, item: Storedata?) {
                ...
                if (coupon.isEmpty()) {
                    ...
                    helper.getView<TextView>(R.id.tvCoupon).visibility = View.GONE
                    helper.getView<TextView>(R.id.tvTextCoupon).visibility = View.VISIBLE
                    helper.setText(R.id.tvTextCoupon, coupon)
                }

                helper.setOnClickListener(R.id.tvText2) {
                    listCoupon.addAll(item!!.coupons)
                    dialogCoupon.show()
                    adapter.notifyDataSetChanged()
                }
            }
...
//                        helper.setText(R.id.tvCoupon, item.face_money)
        ...
        tvPay.onClick {
            if (adrId == 0) {
                ...
                mPresenter.commitOrder(CommitOrderReq(cartid, adrId, couponId))
            }
        }
    }
